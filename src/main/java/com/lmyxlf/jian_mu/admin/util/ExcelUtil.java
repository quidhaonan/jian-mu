...
import com.lmyxlf.jian_mu.admin.handler.ExcelHandlerAdapter;
...
public class ExcelUtil<T> {
    ...
    /**
     * 用于 dictType 属性数据存储，避免重复查缓存
     */
    public Map<String, String> sysDictMap = new HashMap<>();

    /**
     * excel sheet 最大行数，默认 65536
     */
    ...
    public List<T> importExcel(String sheetName, InputStream is, int titleNum) throws Exception {
        ...
        // 如果指定 sheet 名，则取指定 sheet 中的内容 否则默认指向第 1 个 sheet
            ...
            for (int i = titleNum + 1; i <= rows; i++) {
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (ObjUtil.isNotNull(fieldType)) {
                        ...
                        if (StrUtil.isNotEmpty(attr.readConverterExp())) {
                        ...
                        } else if (ColumnType.IMAGE == attr.cellType() && ObjUtil.isNotNull(pictures)) {
                            ...
                            if (image == null) {
                                ...
                                val = FileUtil.writeBytes(data, AdminConfig.getUploadPath());
                            }
                        }
     ...
     * @return 结果
     */
    public LmyXlfResult exportExcel(List<T> list, String sheetName) {
     ...
     * @return 结果
     */
    public LmyXlfResult exportExcel(List<T> list, String sheetName, String title) {
     ...
     * @return 结果
     */
    public LmyXlfResult importTemplateExcel(String sheetName) {
     ...
     * @return 结果
     */
    public LmyXlfResult importTemplateExcel(String sheetName, String title) {
     ...
     * @return 结果
     */
    public LmyXlfResult exportExcel() {
        ...
        try {
            ...
            wb.write(out);
            return LmyXlfResult.ok(filename);
        } catch (Exception e) {

            log.error("导出Excel异常{}", e.getMessage());
            throw new LmyXlfException("导出Excel失败，请联系网站管理员！");
        } finally {
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        ...
        if (ColumnType.STRING == attr.cellType() || ColumnType.TEXT == attr.cellType()) {
            ...
            // 对于任何以表达式触发字符 =-+@ 开头的单元格，直接使用 tab 字符作为前缀，防止 CSV 注入
        ...
        if (StrUtil.isNotEmpty(attr.prompt()) || attr.combo().length > 0 || attr.comboReadDict()) {
            ...
            if (attr.comboReadDict()) {
                ...
                if (!sysDictMap.containsKey("combo_" + attr.dictType())) {
                    ...
                    String labels = DictUtil.getDictLabels(attr.dictType());
                    sysDictMap.put("combo_" + attr.dictType(), labels);
                }
                String val = sysDictMap.get("combo_" + attr.dictType());
                comboArray = StringUtils.split(val, DictUtil.SEPARATOR);
            }

            if (comboArray.length > 15 || StringUtils.join(comboArray).length() > 255) {

                // 如果下拉数大于 15 或字符串长度大于 255，则使用一个新 sheet 存储，避免生成的模板下拉值获取不到
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                } else if (StrUtil.isNotEmpty(dictType) && ObjUtil.isNotNull(value)) {
            ...
            log.error("导出 Excel 失败，e：{}", e.getMessage());
        }
        return cell;
    }
    ...
    private void addStatisticsData(Integer index, String text, Excel entity) {
        ...
        if (entity != null && entity.isStatistics()) {
            ...
            try {
                ...
                log.error("合计统计失败，e：{}", e.getMessage());
            }
    ...
    public Object getCellValue(Row row, int column) {
        ...
        try {
            ...
            log.error("获取单元格值，e：{}", e.getMessage());
            return val;
        }
